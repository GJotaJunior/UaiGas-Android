...
class EditFuelActivity : AppCompatActivity() {
    ...
    fun save(view: View) {
        ...
        when {
            validatorEmpty -> Toast.makeText(
                ...
                RetrofitClient.instance.createFuel(
                    Combustivel(descriptionInput.editText?.text.toString())
                ).enqueue(object : Callback<Combustivel> {
                    override fun onFailure(call: Call<Combustivel>, t: Throwable) {
                        Toast.makeText(
                            applicationContext,
                    ...
                    override fun onResponse(
                        call: Call<Combustivel>,
                        response: Response<Combustivel>
                    ) {
                        when {
                            response.isSuccessful -> {
                                Toast.makeText(
                                    ...
                                    .show()
                                startActivity(Intent(applicationContext, FuelList::class.java))
                            }
                            response.code() == 400 -> {
    ...
    fun update(view: View) {
        ...
        when {
            validatorEmpty -> Toast.makeText(
                ...
                RetrofitClient.instance.updateFuel(
                    id?.toLong()!!,
                    Combustivel(
